...
download_bucket() {
    ...
    for bucket in ${buckets[@]}; do
        ...
        if [ ! -d "${cache_dir}/$bucket_dir" ]; then
...
merge_scripts() {
    rm -rf scripts/*
    for bucket in ${script_buckets[@]}; do
        bucket_dir=$(echo $bucket | sed 's@/@-@g')
        if [ -d "${cache_dir}/${bucket_dir}/scripts" ]; then
...
merge_buckets() {
    ...
    for bucket in ${buckets[@]}; do
        ...
        files_array=(${files})
        if [ ${#files_array[*]} -gt 2000 -a $owner != "ScoopInstaller" ]; then
        ...
        for file in ${files[@]}; do
            ...
            if [ "$?" -eq 0 ]; then
                ...
                if [ ${app_version} == "nightly" -o ${app_version} == "latest" ]; then
                    app_version="0"
                fi
            else
                app_version='0.0.0'
            ...
            if [ "$check_file_md5" -eq 0 ]; then
                echo $file_md5 >>${cache_dir}/file_md5
                if [ "$check_file_id" -eq 0 ]; then
                    ...
                    if [ -f "bucket/${bucket_app_name}" ]; then
                        ...
                        if [ "$?" -eq 0 ]; then
                            ...
                            if [ ${bucket_app_version} == "nightly" -o ${bucket_app_version} == "latest" ]; then
